...
class Level:
    def __init__(self, window: Surface, name: str, game_mode: str, player_score: list[int]):
        self.timeout = TIMEOUT_LEVEL  # equivale a 20 segundos
        self.window = window
        self.name = name
    ...
    def run(self, player_score: list[int]):
        import os
        if os.path.exists(f'./asset/{self.name}.mp3'):
        ...
        while True:
            clock.tick(
                60)  # apresenta quanto de fps será utilizado, no caso, 60 fps (quanto maior o fps, mais rápido irá executar)
